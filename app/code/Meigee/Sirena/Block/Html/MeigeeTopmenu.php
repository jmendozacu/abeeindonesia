 ...
 */
namespace Meigee\Sirena\Block\Html;
...
class MeigeeTopmenu extends Topmenu
{
    ...
    protected function _addSubMenu($child, $childLevel, $childrenWrapClass, $limit)
    {
        ...
        if($is_menu) {
			...
			if($status && $currentCat->getData('meigee_cat_subcontent')) {
                ...
                $html .= '<ul class="back-link-wrapper"><li class="back-link"><a href="javascript: void(0);">' . __('Back to parent') . '</a></li></ul>';
                $html .= $this->getValHtml($currentCat->getData('meigee_cat_subcontent')).'</div>';
			} else {
				if ($status && $childLevel == 1) {
					...
					if($parentCat->getData('meigee_cat_menutype') == 3 || $parentCat->getData('meigee_cat_menutype') == 4){
						...
						$subcontent .= '<li class="back-link"><a href="javascript: void(0);">' . __('Back to parent') . '</a></li><li class="submenu-parent-item"><a href="' . ($catCustLink ? $catCustLink : $child->getUrl()) . '" '  . $target . $boldLink . '> <span>' . $this->escapeHtml(
			                $child->getName()
			            ) . '</span>'.$catLabel.'</a></li>';
				...
				if(!$is_tabs) {
					...
					$html .= '<li class="back-link"><a href="javascript: void(0);">' . __('Back to parent') . '</a></li><li class="submenu-parent-item"><a href="' . ($catCustLink ? $catCustLink : $child->getUrl()) . '" '  . $target . $boldLink . '> <span>' . $this->escapeHtml(
				                $child->getName()
				            ) . '</span>'.$catLabel.'</a></li>';
    ...
    protected function _getHtml(
        ...
        foreach ($children as $child) {
    			...
    			if($status == 1) {
					...
					if ($catImg = $currentCat->getData('meigee_menu_catimg')) {
						...
						if($parentCat->getData('meigee_cat_menutype') !=1){
							...
							$catImgHtml = '<p class="category-image "><img class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" data-src="'.$catImg.'" alt="'.$this->escapeHtml($currentCat->getName()).'" title="'.$this->escapeHtml($currentCat->getName()).'" /></p>';
            ...
            ($child->hasChildren() ? '<a href="javascript: void(0);" class="'.($childLevel == 0 ? 'level-top' : '').' action-link"> <span>' . $this->escapeHtml(
                $child->getName()
            ) . '</span>'.$catLabel.'</a>' : '')
